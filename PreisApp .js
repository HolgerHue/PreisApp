...
// icon-color: green; icon-glyph: shopping-basket;
// Variables used by Scriptable.
// These must be at the very top of the file. Do not edit.
...
// source: https://raw.githubusercontent.com/HolgerHue/PreisApp/main/PreisApp.js; docs: https://raw.githubusercontent.com/HolgerHue/PreisApp/main/README.md; hash: 1143621315;
...
//   • Ablage liegt zusammen in Scriptable/PreisApp (auch eancache.json
//     und meine.json)
//
...
const CHANGELOG = [
  {
    version: "0.69",
    date: "21.08.2026",
    note: "Eine Ablage für beide Teile",
    changes: [
      "eancache.json und meine.json liegen jetzt im Ordner PreisApp, neben items.json",
      "Was im alten Ordner „EAN“ liegt, wandert beim ersten Start herüber; eine Datei, die am Ziel schon liegt, bleibt unangetastet",
      "Der leere Ordner „EAN“ bleibt stehen und kann in der Dateien-App gelöscht werden",
      "Bei aktivem geteilten Ordner liegen die eigenen Einträge mit darin – umstellbar über EAN_DIR im Skript",
      "EAN-Teil 1.01 – nur der Ablageort, Dateiformat unverändert",
    ],
  },
  {
    version: "0.68",
...
// Seine Dateien liegen im Datenordner von PreisApp, eigene Version behält er.
// ═════════════════════════════════════════════════════════════════
...
// eancache.json und meine.json liegen jetzt im Datenordner von PreisApp,
// neben items.json – eine Ablage statt zwei, seit beide Teile in einer
// Datei stecken. Der frühere Ordner „EAN“ wird beim ersten Start einmalig
// geleert: was dort liegt, wandert herüber.
//
// Ist der geteilte Ordner aktiv (SHARED_BOOKMARK), landen die Einträge
// darin und sind für die zweite Apple-ID sichtbar. Wer das nicht will,
// setzt hier fest FM.joinPath(FM.documentsDirectory(), "PreisApp") ein –
// die eigenen Einträge hängen am Open-Food-Facts-Konto, und das ist je
// Person eines.
const EAN_DIR = DIR;
...
/**
 * Umzug aus dem alten Ordner „EAN“. Nur einmal nötig und bewusst
 * vorsichtig: Eine Datei, die am Ziel schon liegt, bleibt unangetastet –
 * lieber eine verwaiste Datei im alten Ordner als ein überschriebener
 * Bestand. Der leere Ordner bleibt stehen, das Löschen von Ordnern
 * überlassen wir der Dateien-App.
 */
(() => {
  const old = FM.joinPath(FM.documentsDirectory(), "EAN");
  if (old === EAN_DIR || !FM.fileExists(old)) return;
  ["eancache.json", "meine.json"].forEach(name => {
    try {
      const from = FM.joinPath(old, name);
      const to = FM.joinPath(EAN_DIR, name);
      if (FM.fileExists(from) && !FM.fileExists(to)) FM.move(from, to);
    } catch (e) {
      console.error("EAN-Umzug: " + name + " – " + e.message);
    }
  });
})();
